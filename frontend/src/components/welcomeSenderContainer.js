...
export class WelcomeSenderContainer extends LitElement {
    static styles = css`
        .container {
        ...
        .container::before {
            ...
            right:  0;
            height: 4px;
            background: linear-gradient(90deg, #4F9DFF, #7B61FF, #FF6B9D);
    ...
    sendWelkomMail() {
        ...
        const naam = this.getFieldValue("Naam");
        ...
        MailService.sendWelcomeMail(email, naam)
            .then((response) => {
                alert("Welkom e-mail succesvol verzonden naar " + naam + "!");
            })
            .catch((error) => {
    ...
    render() {
        return html`
      <div class="container">
          <h2>Welkom email met html content</h2>
          
          <input-field label="Naar" type="email" placeholder="example@mail.com"></input-field>
          <input-field label="Naam" type="text"></input-field>
          
          <button class="send-button" @click=${this.sendWelkomMail}>Verzenden</button>
      </div>
      `
    }
}

customElements.define('welcome-sender-container', WelcomeSenderContainer);